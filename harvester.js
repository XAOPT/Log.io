	...
	*/
	...
	LogStream = (function(_super) {
		...
		LogStream.prototype._watchFile = function(path) {
			...
			conn.exec("tail -f "+path, function (err, stream) {
	...
	LogHarvester = (function() {
		...
		LogHarvester.prototype.run = function() {
			...
			conn.on('ready', function() {
				...
				return _this.logStreams.forEach(function(stream) {
				...
				host: _this.ssh.host,
				port: _this.ssh.port,
				username: _this.ssh.user,
				privateKey: fs.readFileSync('/opt/APP/tribez/.ssh/id_rsa')
			});
